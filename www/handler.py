...
def toc_helper(toc_tokens):
    if toc_tokens:
        return (
            '<div uk-sticky="offset: 200" class="toc"> 文章目录<br> Table of Contents<br><br>'
...
async def get_blog(id):
    ...
    blog.tags = []
    for blog_tag in blog_tags:
        blog.tags.append(dict(blog_tag=blog_tag, tag=await Tag.find(blog_tag.tag_id)))
...
# 编辑日志API
@post("/api/blogs/{id}")
async def api_update_blog(id, request, *, name, summary, content, selectedTags):
    check_admin(request)
    blog = await Blog.find(id)
    ...
    for tag in tags:
        if (await BlogTag.findNumber("count(id)", "`tag_id`=?", [tag.id])) == 1:
            await tag.remove()
    for blog_tag in blog_tags:
...
async def api_get_tag(*, id):
    ...
    if tag:
        tag[0].blogs = list(
            map(lambda x: x.blog_id, await BlogTag.findAll("`tag_id`=?", [tag[0].id]))
        )
    return tag
